...
** Diagnostic Display
#+begin_src emacs-lisp
;; Route flymake diagnostics through eldoc-box instead of flymake's own
;; hover popup (already disabled via flymake-popon :disable t) or the
;; end-of-line summaries.
...
;; Straight underlines instead of squiggly/wave, keeping flymake's own
;; default error/warning/note colors.
(after! flymake
  (custom-set-faces!
   '(flymake-error   :underline (:style line :color "Red1"))
   '(flymake-warning :underline (:style line :color "deep sky blue"))
   '(flymake-note    :underline (:style line :color "yellow green"))))
#+end_src
...
(use-package! flymake-ruff
 ...
 (setq flymake-ruff-program-args
       '("check" "--output-format" "concise" "--exit-zero" "--quiet"
         ;; F821 (undefined-name) is a syntactic guess with no type
         ;; inference; it false-positives on forward references,
         ;; TYPE_CHECKING-only imports, and stub-only symbols.
         ;; pyrefly resolves names correctly via real type analysis,
         ;; so let pyrefly own that diagnostic class and keep ruff's
         ;; flymake report scoped to lint/style/import hygiene.
         "--ignore" "F821" "-"))
  :hook (eglot-managed-mode . flymake-ruff-load))

;; Pyrefly's LSP emits an editor-only "unused-import" note that isn't part
;; of its configurable error-kind set (can't be turned off via
;; pyrefly.toml), and it duplicates ruff's F401. Same situation
;; flymake-ruff's README documents for Pyright's "variable not accessed"
;; notes, with the same fix: filter it out of eglot's flymake report.
;; Ruff keeps this one since it also auto-fixes it.
(defun +eglot-drop-pyrefly-unused-import-a (diags)
  "Drop Pyrefly's `unused-import' notes from DIAGS; ruff's F401 covers it."
  (list (seq-remove
         (lambda (d)
           (string-prefix-p "Pyrefly [unused-import]:"
                             (flymake-diagnostic-text d)))
         (car diags))))
(advice-add 'eglot--report-to-flymake :filter-args
            #'+eglot-drop-pyrefly-unused-import-a)
