# **Mission Objective** You are an expert
...
9.  All workflow files and python code should be inside a workflow subfolder.
...
**A. `run-workflow.sh`**
This bash script starts the LiteLLM proxy via Podman, runs the agentic workflow in the terminal, and then cleans up podman container after workflow finishes  It must use relative paths.

**B. `.envrc`**
The .envrc file for direnv initiates and sets up the python environment. 
...
Design the Python workflow (`main.py` and associated modules) using **Pydantic AI** for agent definition and **LangGraph** for DAG execution. Keep in mind that the python code must be llm model agnostic, meaning the the litellm config may use any model it needs. Direct all LLM calls through the local LiteLLM proxy at `http://localhost:4000`. 
...
**FINAL INSTRUCTION FOR PROMPT 1**: Do not execute the workflow. Output the complete Python application code, the workflow config files, the Bash script contents, the `.gitignore` content, and the `README.md` content. Ensure all logic perfectly aligns with the directory states and constraints provided. Additionally, the workflow must utilize the prompt in the attached markdown file. You must search the web and think longer for all the tasks and the information you get must be the latest till June 2026.
...
(doom!
  :config
  literate
  ;; Modules below were detected by doom-stitcher. Adjust as needed.
  :completion
  company
  :ui
  deft
